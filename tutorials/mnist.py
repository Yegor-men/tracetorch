...
train_dataset = OneHotMNIST(train=True, min_val=0.0, max_val=1.0, offset=0.00)
test_dataset = OneHotMNIST(train=False, min_val=0.0, max_val=1.0, offset=0.00)
...
model_template = snn.Sequential(
	...
	snn.Leaky(8, beta, 1.0, view_tuple=(1, -1, 1, 1)),
	nn.Conv2d(8, 16, 5, dilation=2),
	# kernel 5x5 @ dilation 2 becomes 9x9, 20-9+1 = 12
	snn.Leaky(16, beta, 1.0, view_tuple=(1, -1, 1, 1)),
...
for tr_type in train_types:
	...
	loss_manager = tt.plot.MeasurementManager(f"Loss - {tr_type}", [0.0, 0.9, 0.99])
	accuracy_manager = tt.plot.MeasurementManager(f"Accuracy - {tr_type}", [0.0, 0.9, 0.99])
	...
	for e in range(num_epochs):
		...
		temp_loss, temp_accuracy = 0, 0
		with torch.no_grad():
			for (image, label) in tqdm(test_dloader, total=len(test_dloader), desc=f"{tr_type} - E{e + 1} - TEST"):
				b, c, h, w = image.shape
				image, label = image.to(device), label.to(device)
		...
		# 	print(f"{nn.functional.sigmoid(t)}")
		# model_thresholds = model.get_attr_list("threshold")
		# for t in model_thresholds:
		# 	print(f"{nn.functional.softplus(t)}")
		...
		with torch.no_grad():
			...
			for i, (image, label) in tqdm(enumerate(zip(images, labels)), total=len(images),
										  desc=f"{tr_type} - E{e + 1} - VISUALIZE"):
