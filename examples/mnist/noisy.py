        ...
        with torch.no_grad():
            for image, label in test_dataloader:
                ...
                for t in range(num_timesteps):
                    noise_level = torch.rand_like(image) ** 0.5
                    noisy_image = image * noise_level + (torch.randn_like(image) + 0.5) * (1 - noise_level)
                    running_output += model(noisy_image)
...
if __name__ == "__main__":
    models = {"SNN (LIB)": NoisySNN().to(device), "RNN (GRU)": NoisyRNN().to(device),
              "SSM (S6)": NoisySSM().to(device)}
    results = {}
    ...
    plt.suptitle('Noisy MNIST Classification (traceTorch)')
    plt.tight_layout()
    plt.show()
